...
class Example(Gtk.Window):
    def __init__(self):
        ...
        self.index = 0
        self.init_ui()
        with open('json/keys.json', 'w') as outfile:
    ...
    def _draw(self, cr, index, fill= True):
        cr.set_source_rgb(0.4, 0.4, 0.4)
        if fill:
            cr.rectangle(0, 0, 768, 1024)
            cr.fill()
    ...
    def _save(self):
        for i in range(0, self.tiles.nx * self.tiles.ny):
            ...
            self._draw(context, i, fill=False)
            print "save: {}".format(i)
    ...
    def _save_grid(self):
        ...
        context.set_source_rgb(.75, .75, .75)
        context.fill()
        context = cairo.Context(surface)
        context.set_line_width(1.)
        context.set_source_rgba(.5, .5, .5,.5)
        for i in range(0, self.tiles.nx * self.tiles.ny):
            pos = self.tiles.list[i]
